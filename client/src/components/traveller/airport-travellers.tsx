...
const CONSTANTS = {
  ...
  LABELS: {
    ...
    SELECT_AIRPORT_MSG:
      "Please select an airport above to view available travellers and groups.",
...
const AirportTravellers = () => {
  ...
  type SortOption =
    | typeof CONSTANTS.VALUES.DISTANCE
    | typeof CONSTANTS.VALUES.WAIT_TIME_VAL;
  ...
  useEffect(() => {
    if (!selectedAirport) {
      setTerminals([]);
      setFilterTerminal([]); // Reset terminal filter
      return;
    }
  ...
  const ListSectionWrapper = useCallback(() => {
    ...
    const animation =
      viewMode === VIEW_MODE.INDIVIDUAL
        ? CONSTANTS.ANIMATION.LEFT
        : CONSTANTS.ANIMATION.RIGHT;
    ...
    if (filterGender.length > 0) {
      if (viewMode === VIEW_MODE.INDIVIDUAL) {
        processedTravellers = processedTravellers.filter((t) =>
          filterGender.includes(t.gender),
        );
      } else {
        processedGroups = processedGroups.filter((g) =>
          filterGender.includes(g.gender),
        );
      }
    }
    ...
    if (filterTerminal.length > 0) {
      if (viewMode === VIEW_MODE.INDIVIDUAL) {
        processedTravellers = processedTravellers.filter((t) =>
          filterTerminal.includes(t.terminal),
        );
      } else {
        processedGroups = processedGroups.filter((g) =>
          filterTerminal.includes(g.terminal),
        );
      }
    }
    // 3. SORT
  ...
  const AirportSelect = ({
    ...
    <Dialog open={open} onOpenChange={setOpen}>
        ...
        {children || (
          <Button
            ...
            className={cn(
              ...
              large
                ? "w-full h-14 sm:h-16 text-base sm:text-lg rounded-2xl shadow-sm"
          ...
          >
            <span className="truncate">
              {selectedAirport
                ? airports.find(
                    (airport) => airport.airportName === selectedAirport,
                  )?.airportName
      ...
      <DialogContent
        className="p-0 overflow-hidden bg-background/80 backdrop-blur-xl border-border/20 shadow-2xl rounded-2xl sm:rounded-3xl w-[90vw] max-w-2xl gap-0"
        ...
        <div className="flex flex-col h-[65vh] sm:h-[600px]">
          <div className="px-4 py-3 border-b border-border/10 bg-muted/5">
            ...
            <DialogDescription className="sr-only">
              Search and select your departure airport to connect with fellow
              travellers.
            </DialogDescription>
          </div>
              ...
              <CommandGroup>
                {airports.map((airport) => (
                  <CommandItem
                    ...
                    onSelect={() => {
                      setSelectedAirport(
                        airport.airportName === selectedAirport
                          ? undefined
                          : airport.airportName,
                      );
                      setOpen(false);
  ...
  const ModalWrapper = () => {
    ...
    if (selectedEntity.type === ENTITY_TYPE.TRAVELLER) {
      return <TravellerModal traveller={selectedEntity.data} />;
    }
    if (selectedEntity.type === ENTITY_TYPE.GROUP) {
      return <GroupModal group={selectedEntity.data} />;
    }
  };
    ...
    <>
      ...
      <div className="flex items-center justify-center min-h-[50vh] p-2 sm:p-8 w-full">
        <Card className="w-full max-w-5xl border border-border/20 shadow-xl shadow-black/5 rounded-3xl bg-card/98 backdrop-blur-xl">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <AirportSelect>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-9 w-9 rounded-full opacity-50 hover:opacity-100 hover:bg-primary/10 hover:text-primary transition-all"
                      >
          ...
          <CardContent
            className={cn(
              "px-3 sm:px-10 pb-6 sm:pb-10 transition-all duration-500",
              !selectedAirport
                ? "pt-10 sm:pt-20 pb-20 sm:pb-32"
                : "pt-6 sm:pt-10 space-y-7",
            )}
          >
